...
public class MovieLibrary {
	...
	public Set<Movie> getMoviesWithActor(final String actor) {
		return this.movies.stream().filter(m -> m.getActors().stream().anyMatch(a -> a.couldBe(actor)))
	...
	public Set<Movie> getMoviesByAverageRating(final int rating, final boolean greaterOrEqual) {
		return this.movies.stream()
				.filter(m -> (greaterOrEqual ? m.getAverageRating() >= rating : m.getAverageRating() <= rating))
				// .sorted(Comparator.comparing(Movie::getAverageRating)) // not working!
				.collect(Collectors.toSet());
	}
	...
	public String print(final Set<Movie> movs) {
		...
		movs.stream().sorted(Comparator.comparing(Movie::getTitle)).forEach(m -> sb.append(print(m)).append("\n"));
		return sb.toString();
	}

	public String print(final Movie movie) {
		StringBuilder sb = new StringBuilder();
		sb.append(movie.getTitle()).append(" (").append(movie.getYear()).append(") [").append(movie.getId())
				.append("] Rating: ").append(movie.getAverageRating()).append("/100");
		return sb.toString();
	}
